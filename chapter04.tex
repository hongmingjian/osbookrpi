...
在 ARM早期的版本中，异常向量的基地址只能是0，即复位时 PC=0、 IRQ时 PC=0x18等等，如图\ref{figure:4-8}所示。所以，内核必须保留逻辑地址空间的第一个页面 [0, 0x1000]用于放置异常向量。在 ARM\-720T之后，支持所谓的 HiVecs，即异常向量的基地址可以是 0xffff\-0000。开启 HiVecs后，复位时 PC=0xffff\-0000、 IRQ时 PC=0xffff\-0018等等，见图\ref{figure:4-8}。
...
因此，我们先把 [0xffff\-0000, 0xffff\-1000]这一个页面用 page\_\-alloc\_\-in\_\-addr预留下来，然后用 page\_\-map把它映射到物理地址 0x8000(因为异常向量表位于 kernel.img的开始位置，而 kernel.img被加载到物理地址 0x8000)。这样就把异常向量挪到了逻辑地址 0xffff\-0000上，然后通过修改系统控制寄存器 SCTLR开启 HiVecs模式。
